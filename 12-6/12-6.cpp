...
int main()
{
	...
	long sum_line = 0;//计算队列的总长度
	int wait_time = 0;//一个客人的服务时间
	...
	for (int cycle = 0;cycle < cyclelimit;cycle++)
	{
		if (newcustomer(min_per_cust))//如果有客人来到
		{
			...
			else
			{
				...
				{
					line1.enqueue(temp);//将temp添加到队列末尾
				}
				else
				{
					line2.enqueue(temp);
		...
		if (wait_time <= 0 && !line1.isempty())//如果队列不为空，并且等待时间已经减到0了，则应该出对了
		{
			line1.dequeue(temp);
		...
		if (wait_time <= 0 && !line2.isempty())//如果队列不为空，并且等待时间已经减到0了，则应该出对了
		{
			line2.dequeue(temp);
			wait_time = temp.ptime();
			line_wait += cycle - temp.when();//cycle其实也和实时时间一致，减去到达时间就是在队列种的等待时间
		...
		sum_line += line1.queuecount();//计算队列中的总人数
		sum_line += line2.queuecount();//计算队列中的总人数
	}
	//报告结果
	if (customers > 0)
	{
